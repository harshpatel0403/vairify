...
export default function DateGuardEditGroup() {
  ...
  const debouncedvalue = useMemo(() => {
    return debounce(async (query, otherData = {}) => {
      ...
      try {
        ...
        toast.success(t("dateguardeditgroup.toastSuccess"));
  ...
  useEffect(() => {
    if (!params.groupId) {
      toast.error(t("dateguardeditgroup.toastError"));
      nav(-1);
      return;
    }
    setLoading(true);
    ...
    <>
      <div className="container">
        <div className="md:mb-0 sm:mb-[30px] mb-[16px]">
          <PageTitle title={t("dateguardeditgroup.pageTitle")} />
        </div>
        <div className="flex items-center justify-center">
          <img src="/images/setup/Disarm.svg" alt="guard" />
        </div>
        <div>
          <h3 className="sm:text-[28px] text-[24px] font-semibold text-center text-white my-[24px]">
            {t("dateguardeditgroup.heading")}
          </h3>
        </div>
        ...
        <div className="flex justify-center w-full">
          <input
            ...
            placeholder={t("dateguardeditgroup.inputPlaceholder")}
            type="text"
